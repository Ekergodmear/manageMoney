...
import {
  ...
  saveRecentScenario,
} from '@/features/scenario/scenario-actions';
import { buildScenarioCompareTable } from '@/features/scenario/scenario-compare';
import {
...
} from '@/features/scenario/scenario-engine';
import {
  SCENARIO_QUICK_FORKS,
  type ScenarioExperiment,
  type ScenarioLab,
  type ScenarioLabSnapshot,
} from '@/features/scenario/scenario-types';
...
interface ScenarioPlannerScreenProps {
  ...
  readonly onPromoteSession: (experiment: ScenarioExperiment) => void;
  readonly onPromotePreset: (preset: GamePolicyPreset) => void;
}

function emptyLab(presetId: string): ScenarioLab {
  return {
...
export function ScenarioPlannerScreen({
  ...
  const [lab, setLab] = useState<ScenarioLab>(() => emptyLab(activePresetId));
  const [recent, setRecent] = useState<ScenarioLabSnapshot[]>(() => loadRecentScenarios());
  ...
  const compareTable = useMemo(
    () => buildScenarioCompareTable(lab.experiments),
    [lab.experiments],
  );
  ...
  function patchExperiment(updated: ScenarioExperiment): void {
    setLab((prev) => ({
      ...prev,
  ...
  function addQuickFork(forkId: string): void {
    ...
    const fork = SCENARIO_QUICK_FORKS.find((f) => f.id === forkId);
    if (fork === undefined) {
      return;
    }
  ...
  function loadRecent(snapshot: ScenarioLabSnapshot): void {
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {selectedExperiment.result !== null ? (
                      <>
                        <Button size="sm" onClick={() => onPromoteSession(selectedExperiment)}>
