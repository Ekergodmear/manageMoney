...
export function labToSnapshot(
  lab: {
    ...
    name: string;
    presetId: string;
    baseline: ScenarioExperiment | null;
    experiments: readonly ScenarioExperiment[];
    createdAt: string;
  },
): ScenarioLabSnapshot | null {
...
export function loadRecentScenarios(): ScenarioLabSnapshot[] {
  try {
    const raw = sessionStorage.getItem(RECENT_KEY);
    if (raw === null) {
      return [];
    }
    const parsed = JSON.parse(raw) as (ScenarioLabSnapshot & { branches?: ScenarioLabSnapshot['experiments'] })[];
...
export function saveRecentScenario(snapshot: ScenarioLabSnapshot): ScenarioLabSnapshot[] {
...
export function removeRecentScenario(id: string): ScenarioLabSnapshot[] {
