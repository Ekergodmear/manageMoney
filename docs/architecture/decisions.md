...
Capital và Experiment không trả lời *"tạo session ngay"*.

Chúng trả lời *"đây là các phương án — chọn một"*. Đó là một **lần generate** với:
...
Recommendation = *nên chọn gì*. Candidate = *sẽ commit cái gì*.
...
Continue không phải *"có cách nào tốt hơn không?"* (Improve).

Continue không phải *"với vốn này nên chơi thế nào?"* (Capital).
...
Cùng câu hỏi: *chọn phương án nào trong tập đã generate?*
...
`targetTotalRounds` thay `extendBy` — *"tiếp tục đến 1000"* dễ hiểu hơn *"+500"*.
